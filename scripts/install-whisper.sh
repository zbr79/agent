...
# the multilingual base (q5_1) model so CN + EN dictation both work.
# Run as the ubuntu user on the VM:
#   bash /home/ubuntu/agent/scripts/install-whisper.sh
# Safe to re-run; already-done steps are skipped. ~200 MB disk, no keys needed.
set -euo pipefail

REPO_DIR="${HOME}/whisper.cpp"
MODEL_DIR="${HOME}/models"
MODEL_FILE="${MODEL_DIR}/ggml-base-q5_1.bin"
MODEL_PATH="ggerganov/whisper.cpp/resolve/main/ggml-base-q5_1.bin"
...
echo "== 4/5 model (~183 MB, MIT weights, one file)"
mkdir -p "$MODEL_DIR"
...
If wall clock > ~2x the clip length, swap to the smaller model in .server-env:
  WHISPER_MODEL=\$HOME/models/ggml-base-q5_1.bin   (~74 MB — download the same way)
EOF
